...
var parseQuery = require('./parse-query'),
  parseQueryShape = require('./parse-query-shape'),
  parseOperationStats = require('./parse-operation-stats');
...
var LogEntry = AmpersandState.extend({
    ...
    connection_id: {
      ...
      fn: function() {
        ...
        if (this.thread === 'initandlisten') {
          ...
          if (match = regret('connection.accept', this.message)) {
  ...
  initialize: function() {
    ...
    if (OPS.indexOf(this.tokens[opTypeIndex]) > -1) {
      ...
      var stats;
      if (stats = parseOperationStats(this.tokens, res.pos)) {
